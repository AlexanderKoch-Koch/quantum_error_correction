"""mostly copy from https://github.com/lucidrains/compressive-transformer-pytorch. fixed GruTypegate"""
...
class GruTypeGate(torch.nn.Module):
    def __init__(self, input_size):
        super().__init__()
        self.w_r = nn.Linear(input_size, input_size, bias=False)
        self.u_r = nn.Linear(input_size, input_size, bias=False)
        self.w_z = nn.Linear(input_size, input_size, bias=False)
        self.u_z = nn.Linear(input_size, input_size, bias=False)
        self.w_g = nn.Linear(input_size, input_size, bias=False)
        self.u_g = nn.Linear(input_size, input_size, bias=False)
        self.b_g = 2
...
class Residual(nn.Module):
    def __init__(self, fn):
        super().__init__()
        self.fn = fn
        self.gate = GruTypeGate(64)

    def forward(self, x, **kwargs):
        out = self.fn(x, **kwargs)
        out = cast_tuple(out)
        ret = (out[0] + x), *out[1:]
        # ret = (self.gate(out[0], x)), *out[1:]
        return ret
